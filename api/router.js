...
module.exports = async function handler(req,res) {
  try {
    ...
    if(action==='google_sheet.settings') {
      ...
      let interval=Number(input.sync_interval||60);
      if(![30,60,120,300,600,86400].includes(interval)) interval=60;
    ...
    if(action==='google_sheet.status') {
      ...
      const status={enabled:Number(s.enabled||0),connected:!!s.sheet_url,last_sync_at:s.last_sync_at||null,last_sync_status:s.last_sync_status||null,last_sync_message:s.last_sync_message||null,last_sync_count:Number(s.last_sync_count||0),sync_interval:Number(s.sync_interval||60)};
